...
def creer_evenement_complet(payload: Any, cree_par: str, role: str | None) -> str:
    ...
    created = db.execute(
        ...
        (
            ...
            payload.fenetre_reponse_heures, payload.fenetre_reponse_minutes, payload.fuseau_horaire, description, principal, intervenants, cree_par,  # noqa: E501
        ),
        role=role,
    )
    if not created:
        ...
        for occ in payload.occurrences:
            db.execute(
                ...
                (
                    ...
                    payload.cible_type, cible_id, payload.fenetre_reponse_heures, payload.fenetre_reponse_minutes, payload.fuseau_horaire,  # noqa: E501
                    master_id, cree_par,
                ),
                role=role,
            )
...
def mettre_a_jour_evenement_complet(
    ...
    db.execute(
        ...
        "fenetre_reponse_heures=%s, fenetre_reponse_minutes=%s, fuseau_horaire=%s, description=%s, intervenant_principal=%s, "  # noqa: E501
        "intervenants=%s::text[] WHERE id=%s",
        (
            payload.titre, payload.type, type_evenement_id, payload.volet, payload.debut, payload.fin, payload.lieu,
            payload.mode, primary, liens_json, payload.type_diffusion, payload.visibilite,
            payload.cible_type, cible_id, cible_genre, cible_age_min, cible_age_max, cible_emails,
            payload.fenetre_reponse_heures, payload.fenetre_reponse_minutes, payload.fuseau_horaire, description, principal, intervenants, evenement_id,  # noqa: E501
        ),
        role=role,
    )
    if portee == "toute_la_serie":
        for other in (i for i in serie_ids(evenement_id, portee, role) if i != evenement_id):
            db.execute(
                ...
                "cible_emails=%s::text[], fenetre_reponse_heures=%s, fenetre_reponse_minutes=%s, fuseau_horaire=%s, description=%s, "  # noqa: E501
                ...
                (
                    ...
                    payload.cible_type, cible_id, cible_genre, cible_age_min, cible_age_max, cible_emails,
                    payload.fenetre_reponse_heures, payload.fenetre_reponse_minutes, payload.fuseau_horaire, description, principal, intervenants, other,  # noqa: E501
                ),
                role=role,
            )
...
def ajouter_occurrences_serie(
    evenement_id: str, occurrences: list[Any], actor_id: str, role: str | None
) -> dict[str, int]:
    """Append dates to an activity's series without touching existing occurrences.

    Additive only: a one-off is promoted to a series master on the first append, a
    date already scheduled is skipped, and the series is capped. Shared by the back
    office and collaboration so both append occurrences identically."""
    master = db.fetch_one(
        ...
        "fenetre_reponse_heures, fuseau_horaire, description, intervenant_principal, intervenants "
    ...
    if not master.get("serie_id"):
        db.execute("UPDATE evenement SET serie_id = %s WHERE id = %s", (serie, evenement_id), role=role)
    compte = db.fetch_one("SELECT count(*) AS n FROM evenement WHERE serie_id = %s", (serie,), role=role)
    ...
    for occ in occurrences:
        if total + ajoutees >= _SERIE_MAX:
            break
        if db.fetch_one("SELECT 1 FROM evenement WHERE serie_id = %s AND debut = %s", (serie, occ.debut), role=role):
            continue
        db.execute(
            ...
            "cible_emails, fenetre_reponse_heures, fuseau_horaire, description, intervenant_principal, "
            "intervenants, serie_id, cree_par) "
            "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s::jsonb,%s,%s,%s,%s,%s,%s,%s,%s::text[],%s,%s,%s,%s,%s::text[],%s,%s)",
            (
                ...
                master.get("description"), master.get("intervenant_principal"),
                [str(x) for x in (master.get("intervenants") or [])], serie, actor_id,
            ),
            role=role,
        )
        ajoutees += 1
    ...
    audit.log(actor_id, role, "ajout_occurrences_serie", "evenement", serie,
              {"ajoutees": ajoutees, "total": nouveau_total})
