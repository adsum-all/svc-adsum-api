...
def _rows(monkeypatch):
    def poser(rows):
        faux = _FakeDb(rows)
        monkeypatch.setattr(stats_core, "db", faux)
        return faux
...
def test_every_direction_dimension_is_offered():
    ...
    assert set(stats_core.dimensions_direction()) == {
...
def test_a_known_dimension_returns_rows_in_the_front_s_shape(_rows, dimension):
    ...
    out = stats_core.repartition_globale_par_entite(dimension, None)
    assert out == [
...
def test_an_unknown_dimension_is_refused_rather_than_guessed(_rows):
    faux = _rows([])
    assert stats_core.repartition_globale_par_entite("salaire", None) is None
...
def test_an_unknown_cross_is_refused_too(_rows):
    faux = _rows([])
    assert stats_core.repartition_globale_par_entite("commission", None, "salaire") is None
    assert faux.last_sql == ""


def test_a_cross_builds_a_combined_label_from_both_axes(_rows):
    faux = _rows([{"cle": "Alpha - A", "presents": 4, "partiels": 1, "absents": 0}])
    out = stats_core.repartition_globale_par_entite("commission", None, "volet")
...
def test_the_breakdown_sums_the_same_whatever_the_axis(_rows):
    ...
    par_commission = stats_core.repartition_globale_par_entite("commission", None)
    faux._rows = [
        {"cle": "a", "presents": 9, "partiels": 3, "absents": 4},
        {"cle": "b", "presents": 6, "partiels": 2, "absents": 5},
    ]
    par_pays = stats_core.repartition_globale_par_entite("pays", None)
...
def test_the_query_only_counts_validated_attendance(_rows):
    faux = _rows([])
    stats_core.repartition_globale_par_entite("commission", None)
    ...
    assert stats_core.COMPTE in faux.last_sql
